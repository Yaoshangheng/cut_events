...
do ieq=1,neqmax
   ...
   do is=1,nsta
      do ic=1,icom
         ...
         out_dd=int((t2-t1)/sachead1%delta)+1
         if(nerr.eq.-1)cycle
         ! deltat=mod(msec,int(dt*1000))/1000.0
         n1=int((hour*60.0*60.0+minn*60.0+sec*1.0+msec*1.0/1000.0)/sachead1%delta)+1
         na=n1+int(t1/sachead1%delta)
         nb=n1+int(t2/sachead1%delta)
         out_dd=nb-na+1
         write(*,*)'na=',na
         sig=0
         do i=1,out_dd
            sig(1:out_dd)=sigall(na:nb)
         enddo
